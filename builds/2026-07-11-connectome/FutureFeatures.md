...
6. **Backlink-aware Markdown export** — write `[[wiki-style links]]` back into the original note files pointing at their top related notes, turning Connectome into a one-time enrichment pass over an existing Obsidian-style vault rather than a separate read-only view.
