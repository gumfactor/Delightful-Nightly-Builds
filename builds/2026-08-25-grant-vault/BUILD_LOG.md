...
### [02:15 UTC] Note — example output committed as-is

`grantvault.db` and `grant_vault_dashboard.html`, produced by running the CLI against the two committed fixtures, are committed alongside the source as example output (matching the precedent set by the 2026-06-10 Investment Portfolio Snapshot build, which similarly ships its own generated `report.html`/`watchlist.json`). Both were regenerated after the XSS-test chunk used for the render verification above was removed from the store, so they reflect only the two legitimate fixture documents. A user running the tool against their own data will overwrite both on their first `ingest`/`render`.
