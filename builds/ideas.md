...
| 13 | 2026-08-05 | A | ambitious | AI Model Release Radar | Python CLI/dashboard using the free, no-auth Hugging Face Hub API to track new and trending LLM/agentic-AI model releases relevant to PROFILE.md's named "Agentic AI systems and workflows" and "AI infrastructure" interests — local SQLite snapshot history, dark-mode HTML dashboard with trend charts. Passed over for Impact Ledger tonight: genuinely novel but a passive news-watching tool with a weaker "saves real time on a recurring task" case than a grant/manuscript-writing-focused build. | — | — | — | pending |
| 14 | 2026-08-05 | A | ambitious | Canadian Sector Ownership Dashboard | Python CLI/dashboard combining Bank of Canada Valet API + StatsCan WDS + Wikidata to show macro-level Canadian-vs-foreign ownership by industry sector, tied to The Canada List's mission. Passed over for Impact Ledger tonight: CanEcon Pulse (2026-07-18) already occupies the "Canadian macro dashboard" shape in Category A — would need a more distinct angle before building to avoid reading as a close sibling rather than a clearly separate build. | — | — | — | pending |
| 15 | 2026-08-06 | B | ambitious | Grant Budget & Compliance Tracker | Tracks multiple active grants (funder, award amount, period, categories) against a manually-logged expense ledger, computes under/overspend pace relative to the elapsed award period, flags grants at risk, and drafts progress-report-ready summary text. Passed over for Manuscript Pipeline tonight: the core signal is purely user-entered arithmetic with no live external verification layer, a weaker differentiator than Manuscript Pipeline's Crossref auto-detection. | — | — | — | pending |
| 16 | 2026-08-06 | B | ambitious | Response-to-Reviewers Letter Builder | Paste reviewer comments, get an AI-assisted point-by-point response draft with per-comment resolution-status tracking, exports a formatted response letter. Passed over for Manuscript Pipeline tonight: genuinely useful but single-session in nature (used once per revision, not a recurring daily/weekly tool per PROFILE.md's ranked build-value criteria) — better scoped as a `respond` subcommand added to Manuscript Pipeline in a future session than as a standalone build. | — | — | — | pending |
