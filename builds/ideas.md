...
| 13 | 2026-06-23 | C | ambitious | Lab Project Context Vault | SQLite-backed knowledge store for all active research projects: each project has a title, current goal, recent decisions, blockers, and next actions. Anthropic API generates a structured current-state brief from recent log entries. HTML viewer shows project timelines with AI-generated summaries. Directly addresses "context loss between AI coding sessions" but for research specifically. | — | — | — | pending |
| 14 | 2026-06-23 | C | ambitious | Grant Evidence Collector | Per-grant citation organizer: add DOIs, auto-populate title/abstract from CrossRef API (free, no auth), tag citations to specific aims/sections. Anthropic API synthesizes what papers in each section collectively show. Generate exportable literature review drafts in markdown. Useful during active grant writing periods. | — | — | — | pending |
