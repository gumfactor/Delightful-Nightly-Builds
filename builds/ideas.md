...
| 9 | 2026-06-19 | H | ambitious | Git History Visualizer | Python CLI that reads the git log of any local repo and generates a self-contained HTML report with: commit frequency heatmap (GitHub contribution graph style), per-author contribution breakdown, most-churned files, and peak development periods. Pure stdlib (subprocess + git log parsing). | — | — | — | pending |
| 10 | 2026-06-19 | H | ambitious | Python Test Coverage Gap Finder | Python CLI that compares source files in src/ against test files in tests/ to identify: which source modules have no corresponding test file, which functions are never imported in any test, and which test files have the lowest assertion density. Pure stdlib — no coverage.py dependency. | — | — | — | pending |
