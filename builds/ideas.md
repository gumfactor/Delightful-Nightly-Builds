...
| 23 | 2026-07-30 | D | ambitious | EchoScape | Ambient soundscape generator sonifying the user's own live GitHub commit activity into a "coding session soundtrack" (Web Audio synthesis keyed to today's commit velocity and file-type mix, with a synchronized Canvas visual) | — | — | Considered and passed over for the 2026-07-30 build (Vizstract) — reuses WeatherSong's (2026-07-03) exact mechanic (live data deterministically mapped into Web Audio + Canvas) on a new data source rather than a new mechanic, and GitHub is already the backbone of a large share of the catalog (Worklog, Pipeline Pulse, ci-pulse, BugTrace, two Developer Analytics/Activity dashboards). Revisit only if paired with a genuinely new sonification technique, not just a new data source. | pending |
| 24 | 2026-07-30 | D | ambitious | Lab Fable | Branching interactive-fiction generator that turns a real published finding (looked up by DOI via the free Crossref API) into a short branching teaching scenario for students | — | — | Considered and passed over for the 2026-07-30 build (Vizstract) — structurally a third "combinatorial content generator seeded from an academic corpus" build (after Research Question Forge and Bridgework) wearing a narrative skin, the same architecture-reuse concern that ruled out repeating Bridgework's pattern a third time. Revisit with a genuinely distinct generation mechanic, not another taxonomy-cross-product variant. | pending |
