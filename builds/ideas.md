...
| 13 | 2026-06-30 | A | ambitious | Kwyeter Noise Profile Visualizer | Browser dashboard visualising environmental noise profiles for venues — integrates with Kwyeter's data model to show noise distributions by time-of-day and day-of-week, compare against safe exposure thresholds, and surface venues that are particularly loud or quiet; targets the core value proposition of Kwyeter | — | — | — | pending |
| 14 | 2026-06-30 | A | ambitious | Canada List Category Intelligence Dashboard | Uses Statistics Canada open data to visualise which product categories are most dominated by foreign vs. Canadian businesses — publication counts by sector, province-by-province market share estimates, and editorial priority signals for The Canada List curation team | — | — | — | pending |
| 15 | 2026-07-01 | B | ambitious | Manuscript/Grant AI Consistency Checker | Batch-scans a folder of manuscript/grant draft files (.docx/.md/.txt) with Claude to flag terminology drift across sections, undefined acronyms, and missing citation placeholders; outputs a per-file report plus a cross-file consistency summary | — | — | — | pending |
| 16 | 2026-07-01 | B | ambitious | Research Ethics/IRB Protocol Batch Assembler | Given reusable boilerplate sections (consent language, data security statements, recruitment scripts) and a target study config, batch-assembles a first-draft REB/IRB submission document and flags sections still needing custom input | — | — | — | pending |
| 17 | 2026-07-02 | C | ambitious | Personal Manuscript/Grant Knowledge Base | Ingest the user's own manuscript/grant drafts and build a searchable, AI-extracted concept index (claims, methods, defined terms) across their own writing, so they can query across their own body of work | — | — | Manual-entry pattern (no bundled real content to seed or test against) — same failure mode as the 2026-06-06 AI Session Context Bridge, which scored 3/10 for the same reason. Worth revisiting if paired with a real auto-ingestion source (e.g. watch a folder of already-written files) rather than requiring upfront manual curation. | pending |
| 18 | 2026-07-02 | C | ambitious | Canada List Classification Precedent Log | Searchable local database of prior "is this business Canadian-owned" classification decisions with AI-assisted rationale extraction, to speed up editorial QC for The Canada List | — | — | No real classification data exists yet to seed or test against; better suited to category F (Data Explorer) once real Canada List export files are available to work from. | pending |
