...
def compute_quality(
    ...
    timing_threshold: responses shorter than this (in seconds) are flagged as fast.
    """
    rows = survey.rows
    ...
    for scale_name, cols in detected_scales.items():
        complete_rows = [
            r for r in rows if all(r.get(c) is not None for c in cols)
        ]
        ...
        try:
            item_scores = [
                [float(r[c]) for r in complete_rows]
                for c in cols
            ]
